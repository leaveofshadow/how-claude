...
## loop 适用性自检（四条件劝退门，文章 L1/L2/L3）

路由到 cc-loop 前，先过四条件——任一不满足 → 劝退用普通会话（loop 成本 > 收益）：

| 条件 | 判据 | 不满足 → |
|---|---|---|
| 任务重复 | 至少每周一次（摊销搭建成本）| 一次性任务 → 用好 prompt，别搭 loop |
| 自动验证 | 有东西能拒绝烂输出（测试/类型检查/linter/构建）| 无自动闸 → 回椅子读 diff，loop 空转 |
| 端到端 | agent 能自己干完整活，不把一半还给你 | 需人坐椅子的活 → 不适合 loop |
| 客观完成 | "完成"是客观的（命令+期望），非判断题 | 质量是口味问题 → 人类还是赢 |

**触发**：用户表达「循环/自动化/定时/loop/ralph/babysit」类 → 路由 cc-loop 前，先查这 4 条。漏一条 → 建议普通会话 + 好 prompt（5 分钟能做完的不需要 loop）。

判据（可证伪）：4 条各枚举可检（重复频率 / 验证闸存在 / 端到端能力 / 完成客观性）；非「看起来适合」。
...
> **ultracode / fan out subagents**（Workflow 工具 opt-in 关键词）分流：
> - 设计决策语境（"ultracode 帮我对抗设计/判官小组/方案对比 X"）→ **cc-2pp**（模式C Workflow 增强，内置判官小组+对抗）
> - 通用多 agent 编排（并行审查/研究/迁移，非设计决策）→ **cc-orchestration** 或直接 Workflow 工具
