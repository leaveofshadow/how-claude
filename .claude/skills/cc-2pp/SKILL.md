...
2. **内容=生成** → 实施成本是 token/上下文/验证，不是"人天"。ROI 的投入侧按此算。
...
> **技术选型 = web 搜索 × 多视角 × ROI，三位一体。** 少一个都漏：没 web→幻觉；没多视角→盲区；没 ROI→性价比灾难。（来源：RA2 演练，攻击者事后才挖出 ROI 烂——现前置为检查点）
...
**ROI 触发规则（原则 2/3 的操作化）：** 在【架构设计】或【核心技术选型】决策点，必须处理 ROI（询问用户 / 帮用户建模投入产出比）。绝不在 ROI 真空里做架构/技术决策。落点见 Phase 0c / Phase 2 评分 / Phase 4 / 攻击者 C。
...
```
★ 实施者认知锚定（假设1 认知层——先钉死"谁干活"，再评估任何方案）
  开始起草/攻击前，先在心里钉死一句: 实施者 = Claude Code + 已装载 skills，不是人类团队。
  这句钉死后，你评估的是"Claude + skills 能不能落地"，不是"团队能不能排期":
    · 方案里每个"谁来做" = Claude/agent，不是岗位/人
    · 攻击向量 = Claude 的能力边界、缺什么 skill、验证能否自闭环，
      不是"团队有人会吗、学习曲线多陡、多久上手"
    · 校准失败的信号: 冒出"学习成本/人手/排期/团队熟练度" → 立刻停，换回 Claude 视角
...
★ 按需补齐方法论（假设1 推论1 执行版——缺认知先读 cc-* 子技能，不是读路由器）
  ...
  定位的对应 cc-* 子技能深度参考，补齐认知再动手:
    · 涉及自动循环        → cc-loop/references/loop-guide.md（循环合同/护栏三件套/闭环反馈）
    · 涉及验收标准        → cc-goal/references/goal-guide.md（五层模型/自评通过/预检）
    · 涉及多 agent 编排   → cc-orchestration/references/orchestration-guide.md（决策树/编排循环）
    · 涉及工程配置/锚文件 → cc-config/references/config-systems-guide.md（六层配置/锚文件）
  没读对应子技能就攻击 → 攻击浮于表面（违反 effort=max）
  ⚠️ 不读 claude-coach 路由器——它是分诊台不是知识库，深度内容在具体子技能里

★ Claude 实施者度量（假设1 下沉——反"人天"默认值）
  实施者是 Claude + skills，不是人类团队。工作量估算禁用"人天/人周/人月"，
  必须用以下度量:
    · token 成本（输入+输出，含 retry 预算）
    · 上下文轮次（需几轮对话/几个 agent 并行）
    · skill 配置成本（要新造/装载哪些 skill，联动 cc-scanner）
    · 验证复杂度（验收条件多可证伪、需多少测试/检查）
    · 依赖与风险（外部 API/数据/人工确认点）
  ROI 投入侧按上述算，产出侧照常（价值/收入/学习）。

★ 其他必注入约束（摘自假设1/2，反 LLM 缺陷）
    · 不走捷径、不省步骤（effort=max，反偷懒）
    · 事实声明必须有证据来源（反幻觉）
    · 缺能力先找/造 skill，不算"团队学习成本"（假设1 推论1）
    · 方案必须列出"需要哪些 skills"（假设1 推论4，消费 Phase 0 能力清单）
    · ★保姆级细化（Phase 4 产出，反"假设 Claude 能猜"）: 每个需求项/步骤写到"新实例只看这一条就能动手"；
      禁"实现 X 功能"式模糊；验证必须可证伪（命令+期望输出），不留"适当/合理"
```

> 评分阶段（30-score.md）必须检查每个方案/计划**是否违反上述约束**——出现"人天/人周/学习成本/排期"，或任何隐含"人类团队是实施者"的表述（角色分工/人岗/团队熟练度/学习曲线），即判**认知锚定失败**扣分（假设 1+2）。
...
- 可行性/可维护性/可扩展性/实现成本/用户价值: 各 0-14 + 一句理由
  （实现成本必须按 Claude 实施者度量: token/轮次/skill配置/验证；出现"人天/人周"即扣分）
...
Phase 0: Explore（必做，不可跳过——假设1/2/原则1）
    ...
    │  → 编排者把返回结果落盘到 .2pp/{run}/00-explore.md
...
Phase 2: Plan（一等公民：agent 写文件，编排者读文件——假设4）
    ...
    │    Agent(subagent_type: "general-purpose", model: "opus") 起草主力方案 → 写 .2pp/{run}/10-plan-main.md
    ...
    │      ★ C 的攻击向量必含 ROI（投入产出比合理吗？）+ ★工作量是否按 Claude 实施者度量（出现人天/人周即攻击点）+ ★方案是否隐含人类实施者假设（角色分工/人岗/学习曲线→攻击点）+ ★可编排性（方案能否拆成可独立验证、≤2 并发的 worktree 工作单元？验证闸是否清晰可证伪？不能拆→下游 60/70 无法编排→攻击点）
...
Phase 3: Output & Persist
    ...
    └─ 持久化决策记录 → .2pp/{run}/50-decision.md（或用户指定目录）
    │
    ▼
