...
2. **内容=生成** → 实施成本是 token/上下文/验证，不是"人天"。token 可度量（仍用于实施成本估算），但 token ROI 不好估（见假设5），方案决策改用三项好估维度（功能需求/技术选项/运维成本）。
...
⚠️ **token ROI 降级（Boss 方法论修正）：** Claude 实施者的 token ROI **不好估**（投入侧 = token 是 Claude 运行成本，非用户直接感知；产出侧 = 规格/闸/契约的价值难提前货币化；两端都模糊）→ **不作方案比较/裁决的主轴**。token 仍作实施成本度量（见 Prompt 注入约束），但不作决策依据。落点见 Phase 0c / Phase 2 评分 / Phase 4 / 攻击者 C。
...
★ Claude 实施者度量（假设1 下沉——反"人天"默认值）
  ...
  ⚠️ **token ROI 降级（Boss 方法论修正）**：上述度量用于估算实施成本，但 token ROI（投入产出比）不好估（投入侧 = Claude 成本非用户感知，产出侧难货币化）→ **不作方案比较/裁决主轴**。方案裁决用三项好估维度（功能需求 / 技术选项 / 运维成本全生命周期，见假设5）。
