...
★ 其他必注入约束（反 LLM 缺陷）
    ...
    · ★禁用模糊词（charter §1 总则6 + contracts/terminology.md §禁令清单）:
      产出（方案/攻击/计划/需求）禁用纯模糊词（最懒/努力/充分/拼命/深入/尽快/合理/适当/空中楼阁/灵魂/一等公民/保姆级/真综合/轻量/深度/完整度/系统性/足够/快速/最快/更细/最优/最佳/完美），改用可证伪表述（命令+期望输出 / 可枚举检查项）；半模糊词（闭环/漂移/锚点/加严）必须附操作定义。已附操作定义的词可留（如本条"保姆级"已跟"每条6字段+可证伪验证"定义、"effort=max"已是步骤完整性4条清单）。
    · ★数理推导 + 量化（技术选型必含，阶段1②③）:
      技术选项禁纯定性理由——必含数理推导（性能排队论/利特尔、扩展 Amdahl/大O、容量负载模型、可靠性概率 MTBF、可行域约束，给公式+数值结论+下界）；量化数值必标来源（web benchmark / 官方 spec / POC 实测，防幻觉）。Phase 3.5 POC 指标（性能 p99/QPS + 正确性 错误率/边缘 case + 可靠性 MTBF + 成本 + 集成）必给阈值（来自②推导理论值）+ 统计（样本量/置信区间/多次跑分布）。纯定性"够用/合理"= 偷懒，扣分。
```
