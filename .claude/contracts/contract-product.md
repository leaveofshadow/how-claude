...
**[层3 已装配]**：
- **venture-product-requirement**（PRD 治理）：✓ 装配（M2）—— N3.5 产 PRD（条目化 AC{n}/R{n}）+ N5 核实走偏（被动 review 开发部实施，§4.1 R3.2 详）+ 变更回流（`N3.5_需求变更_request.md` / `_changelog.md`）。

**[层3 待装配]**：venture-product（产品设计）/ venture-product-uiux（UIUX 设计，引 Bergside Type UI）—— 层3 cc-venture 新建（charter L80 真空标注；venture-product-uiux 沙箱验证 M5，项目级待 deploy）。00-explore §四产品部 ❌ 真空。
